...
if [ -z "$MACHINES" ]; then
    # Local parallel
    export SYSC_MPI_CMD="${I_MPI_ROOT}/intel/bin/mpiexec.hydra -n ${NPROCS}"
else
   ...
   (IFS=','; for HOST in $MACHINES; do echo "$HOST" >> $INTEL_MPI_MACHINESFILE; done)
    ...
    export SYSC_MPI_CMD="${I_MPI_ROOT}/intel/bin/mpiexec.hydra -machinefile $INTEL_MPI_MACHINESFILE"
fi
